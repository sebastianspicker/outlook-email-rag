...
class EmailRetriever:
    ...
    def search_filtered(
        ...
        """
        # ── Pre-filter: resolve semantic constraints ─────────────────
        ...
        has_filters = bool(
            ...
            or email_type
            or allowed_uids is not None
            or category
            or is_calendar is not None
            or attachment_name
            or attachment_type
        )

        # ── Compute overfetch size ────────────────────────────────────
        settings = getattr(self, "settings", None)
        ...
        # ── Iterative fetch-filter-dedup loop ─────────────────────────
    ...
    def search_by_thread(self, conversation_id: str, top_k: int = 50) -> list[SearchResult]:
        ...
        # Use collection.get() with a metadata filter instead of query() with a
        # dummy embedding.  This avoids cosine-distance bias from a synthetic
        # vector and is the intended ChromaDB API for non-semantic lookups.
        conv_filter = {"conversation_id": {"$eq": conversation_id.strip()}}
        # Fetch in pages to avoid truncation when threads have many chunks.
        # Each email may have multiple chunks, so we need to fetch enough
        # to cover top_k unique emails after deduplication.
    ...
    def format_results_for_claude(
        ...
        *max_body_chars* and stops emitting results when *max_response_tokens*
        ...
        # Maintain running token count instead of re-estimating the entire
        # output on every result (avoids O(n^2) behaviour).
        running_tokens = sum(estimate_tokens(p) for p in parts)
    ...
    def _expand_query(self, query: str) -> str:
        ...
        try:
            ...
            if self._query_expander is None:
                ...
                self._query_expander = QueryExpander(model=self.model, vocabulary=vocab)

            return self._query_expander.expand(query, n_terms=3)
        except Exception:
