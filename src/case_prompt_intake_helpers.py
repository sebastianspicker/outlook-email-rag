...
_RANGE_FROM_RE = re.compile(r"(?i)\bfrom\s+([a-zäöüA-ZÄÖÜ]+(?:\s+20\d{2})?|20\d{2}-\d{2}-\d{2})")
_RANGE_TO_RE = re.compile(r"(?i)\bto\s+([a-zäöüA-ZÄÖÜ]+(?:\s+20\d{2})?|20\d{2}-\d{2}-\d{2}|present|today|now)")
...
_FOCUS_RULES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("discrimination", ("discrimination", "benachteiligung", "agg", "disability disadvantage")),
...
_TRIGGER_CANDIDATE_RULES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("escalation_to_hr", ("hr", "human resources", "personalabteilung", "escalation to hr")),
...
_ADVERSE_ACTION_CANDIDATE_RULES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("project_removal", ("project removal", "removed from project", "projekt entzogen", "project withdrawal")),
    ("mobile_work_restriction", ("mobile work restriction", "home office restriction", "remote work denied", "home office")),
    ("attendance_control", ("attendance control", "surveillance", "novatime", "worktime control", "arbeitszeitkontrolle")),
...
_MISSING_RECORD_RULES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("novatime_attendance_records", ("novatime", "attendance", "timesheet", "arbeitszeit")),
...
def _extract_dates(prompt_text: str, *, today: str, assume_date_to_today: bool) -> dict[str, Any]:
    iso_dates = [match.group(1) for match in _ISO_DATE_RE.finditer(prompt_text)]
    month_dates = [_month_year_to_iso(match.group(1), match.group(2)) for match in _MONTH_YEAR_RE.finditer(prompt_text)]
    explicit_dates = list(dict.fromkeys([*iso_dates, *month_dates]))
...
def _named_people(prompt_text: str) -> dict[str, list[dict[str, Any]]]:
    emails = list(dict.fromkeys(match.group(1) for match in _EMAIL_RE.finditer(prompt_text)))
    ...
    for role, pattern in _NAMED_ROLE_PATTERNS:
        ...
        for match in pattern.finditer(prompt_text):
            ...
            row = {
                "name": name,
                "email": next((email for email in emails if name.split()[0].lower() in email.lower()), None),
