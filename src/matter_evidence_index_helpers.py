...
_ADVERSE_ACTION_TEXT_HINTS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("attendance control", ("novatime", "attendance control", "worktime control", "arbeitszeitkontrolle")),
)
...
def issue_tags(case_bundle: dict[str, Any], source: dict[str, Any], findings: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for tag_id in text_to_issue_tag_ids(direct_text):
        append(
            tag_id,
            assignment_basis="direct_document_content",
            evidence_status="directly_supported",
            reason="Tag keywords are directly visible in the current source text.",
        )
...
def why_it_matters(source: dict[str, Any], findings: list[dict[str, Any]]) -> str:
    ...
    return "Provides direct record material relevant to the current matter review."
...
def sender_identity(source: dict[str, Any]) -> dict[str, str]:
    source_type = str(source.get("source_type") or "")
    if source_type == "email":
...
def sender_or_author(source: dict[str, Any]) -> str:
    identity = sender_identity(source)
    if identity:
...
def recipient_identities(source: dict[str, Any]) -> dict[str, list[dict[str, str]]]:
    source_type = str(source.get("source_type") or "")
    if source_type == "email":
        return {
            "to": [
...
def recipients(source: dict[str, Any], source_lookup: dict[str, dict[str, Any]], source_links: list[dict[str, Any]]) -> list[str]:
    identities = recipient_identities(source)
    values = [
...
def source_language(source: dict[str, Any]) -> str:
    documentary_support = as_dict(source.get("documentary_support"))
    return detect_source_language(source.get("title"), source.get("snippet"), documentary_support.get("text_preview"))
