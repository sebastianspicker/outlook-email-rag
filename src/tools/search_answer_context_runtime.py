"""Runtime orchestration for answer-context payload assembly."""
...
from .search_answer_context_rendering import _answer_policy, _answer_quality, _final_answer_contract, _render_final_answer
...
async def build_answer_context_payload(deps: ToolDepsProto, params: EmailAnswerContextInput) -> dict[str, Any]:
    ...
    def _run() -> dict[str, Any]:
        ...
        if budget > 0 and _estimated_json_chars(final_payload) > budget:
            ...
            def _trim_snippet(text: Any, *, max_chars: int) -> str:
                collapsed = " ".join(str(text or "").split())
                if len(collapsed) <= max_chars:
                    return collapsed
                return collapsed[: max_chars - 3].rstrip() + "..."

            def _trim_provenance(provenance: Any) -> dict[str, Any]:
            ...
            def _trim_candidate(item: Any) -> dict[str, Any]:
                ...
                trimmed = {
                    ...
                    "snippet": _trim_snippet(item.get("snippet"), max_chars=120),
                    "provenance": _trim_provenance(item.get("provenance")),
                }
            ...
            final_payload["candidates"] = [_trim_candidate(item) for item in list(final_payload.get("candidates") or [])]
            final_payload["attachment_candidates"] = [
                _trim_candidate(item) for item in list(final_payload.get("attachment_candidates") or [])
            ]
        ...
        if budget > 0 and _estimated_json_chars(final_payload) > budget:
            for item in list(final_payload.get("candidates") or []):
                if isinstance(item, dict):
                    item["snippet"] = _trim_snippet(item.get("snippet"), max_chars=48)
