    ...
    async def email_temporal(params: EmailTemporalInput) -> str:
        ...
        def _work(db):
            ...
            if params.analysis == "volume":
                return json_response(analyzer.volume_over_time(
            ...
            if params.analysis == "response_times":
                return json_response(analyzer.response_times(
                    sender=params.sender, limit=params.limit,
                ))
            return json_error(
                f"Invalid analysis: {params.analysis}. Use 'volume', 'activity', or 'response_times'."
            )
