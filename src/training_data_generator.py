...
class TrainingDataGenerator:
    ...
    def generate_triplets(
        ...
        """
        threads = self._load_threads(min_thread_size)
        if not threads:
            logger.warning("No threads with >= %d emails found", min_thread_size)
            return []

        sender_index = self._build_sender_index()
        all_emails = self._load_all_emails()
            ...
            for i, query_email in enumerate(emails):
                for j, pos_email in enumerate(emails):
                    ...
                    neg_text = self._find_negative(
                        query_email, conv_id, sender_index, all_emails, max_passage_len,
                    )
                    if not neg_text:
                        continue
    ...
    def _build_sender_index(self) -> dict[str, list[dict]]:
        ...
        rows = self._db.conn.execute(
            "SELECT uid, conversation_id, sender_email, subject, body_text FROM emails"
        ).fetchall()
    ...
    def _find_negative(
        ...
        if sender in sender_index:
            candidates = [
                e for e in sender_index[sender]
                if e["conversation_id"] != query_conv_id
                and (e["body_text"] or "").strip()
            ]
            if candidates:
