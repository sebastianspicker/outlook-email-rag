...
_REQUEST_RE = re.compile(
    ...
    r"koennten sie|könnten sie|send|confirm|share|provide|reply|respond|acknowledge)\b"
)
...
def _request_expected(text: str) -> tuple[bool, list[str]]:
    reasons: list[str] = []
...
def build_reply_pairing_index(
    *,
    ...
    for row in rows:
        request_expected, request_reasons = _request_expected(row["text"])
