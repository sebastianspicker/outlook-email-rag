...
class WritingAnalyzer:
    ...
    def analyze_sender_profile(
        self, texts: list[str], sender_email: str = ""
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "emails_analyzed": n,
            "avg_readability": _safe_avg(
                [m.readability_score for m in metrics_list if m.readability_score is not None]
            ),
            "avg_grade_level": _safe_avg(
                [m.grade_level for m in metrics_list if m.grade_level is not None]
            ),
            "avg_sentence_length": round(
                sum(m.avg_sentence_length for m in metrics_list) / n, 2
            ),
            "avg_word_length": round(
                sum(m.avg_word_length for m in metrics_list) / n, 2
            ),
            "avg_vocabulary_richness": round(
                sum(m.vocabulary_richness for m in metrics_list) / n, 4
            ),
            "avg_formality": round(
                sum(m.formality_score for m in metrics_list) / n, 4
            ),
