...
_GROUP_RULES: tuple[dict[str, Any], ...] = (
    ...
    {
        "group_id": "novatime_attendance_records",
        "title": "NovaTime / Attendance Records",
        "keywords": ("novatime", "attendance", "time record", "working time", "timesheet"),
    ...
    if "comparator_actors" in missing_fields:
        ...
        group["items"].append(
            _request_item(
                ...
                likely_custodian=str(rule["custodian"]),
                would_prove_or_disprove=(
                    "Would prove or disprove whether the claimant was treated differently "
                    "from similarly situated peers."
                ),
                urgency=str(rule["urgency"]),
                risk_of_loss=str(rule["risk_of_loss"]),
                preservation_action=str(rule["preservation_action"]),
    ...
    if "org_context" in missing_fields:
        ...
        group["items"].append(
            _request_item(
                ...
                likely_custodian=str(rule["custodian"]),
                would_prove_or_disprove=(
                    "Would prove or disprove decision authority, gatekeeping position, "
    ...
    if "alleged_adverse_actions" in missing_fields or "retaliation_focus_without_alleged_adverse_actions" in {
        ...
        group["items"].append(
            _request_item(
                ...
                likely_custodian=str(rule["custodian"]),
                would_prove_or_disprove=(
                    "Would prove or disprove whether adverse treatment intensified "
