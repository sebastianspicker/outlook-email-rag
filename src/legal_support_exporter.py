...
def _counsel_export_readiness(export_metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    review_classification = _as_dict(export_metadata.get("review_classification"))
    persisted_snapshot_present = any(
        str(export_metadata.get(field) or "").strip() for field in ("snapshot_id", "workspace_id", "matter_id")
    )
    counsel_grade_review = bool(
        review_classification.get("may_be_presented_as_full_matter_review")
        or str(review_classification.get("counsel_use_status") or "") == "counsel_grade_exhaustive_review"
    )
    if review_state not in {"human_verified", "approved"} and not (not persisted_snapshot_present and counsel_grade_review):
