...
def _matter_evidence_index_section(
    *,
    ...
    for index, row in enumerate(rows[:4], start=1):
        ...
        strength_label = str(exhibit_reliability.get("strength") or row.get("reliability_or_evidentiary_strength") or "")
        readiness_label = str(next_step_logic.get("readiness") or "readiness_unknown")
        entries.append(
            {
...
def _lawyer_issue_matrix_section(
    *,
    ...
    entries = [
        ...
        for index, row in enumerate(matrix_rows[:4], start=1)
    ]
    section = _section_with_entries(
        section_id="lawyer_issue_matrix",
        title="German Employment Lawyer Issue Matrix",
