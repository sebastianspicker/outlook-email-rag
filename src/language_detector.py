...
def detect_language(text: str) -> str:
    ...
    # Use frequency-weighted matching: a stopword appearing 10 times is
    # stronger evidence than one appearing once.  Counter gives us the
    # actual token frequencies rather than just unique occurrences.
    ...
    for lang, stopwords in _STOPWORDS.items():
        # Sum the frequencies of all tokens that match stopwords
        weighted_matches = sum(token_counts[word] for word in stopwords if word in token_counts)
        # Normalize by total token count and stopword set size
        score = (weighted_matches / total_tokens) if total_tokens else 0
        if score > best_score:
            best_score = score
            best_lang = lang
