...
async def test_email_search_structured_exposes_retrieval_diagnostics(monkeypatch):
    ...
    class DummyRetriever(_BasicRetriever):
        ...
        def last_search_debug(self):
            return {
                "original_query": "student.case@example.edu",
                "executed_query": "student.case@example.edu sbv timeline",
    ...
    params = EmailSearchStructuredInput(query="student.case@example.edu", expand_query=True)
    payload = await email_search_structured(params)
    data = json.loads(payload)

    assert data["retrieval_diagnostics"]["original_query"] == "student.case@example.edu"
...
@pytest.mark.asyncio
async def test_email_ingest_updates_runtime_archive_paths_for_subsequent_reads(monkeypatch):
    from src.mcp_models import EmailIngestInput
    from src.tools.search import email_ingest
    ...
    mock_set_paths.assert_called_once_with(
        chromadb_path="/tmp/custom-chroma",
        sqlite_path="/tmp/custom-email.db",
    )
