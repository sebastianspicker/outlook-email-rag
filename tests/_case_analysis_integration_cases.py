...
async def test_build_case_analysis_augments_multi_source_bundle_with_native_chat_exports(monkeypatch, tmp_path) -> None:
    ...
    assert chat_log["participants"] == ["Max Mustermann", "Erika Beispiel"]
    assert chat_log["chat_message_count"] == 2
    assert chat_log["chat_message_units"][0]["speaker"] == "Max Mustermann"
        ...
        "artifacts": [
            {
                ...
                "text": "Max Mustermann raised a formal complaint to HR and SBV.",
            },
            {
                ...
                "text": "Max Mustermann was denied home office after the complaint.",
            },
            {
...
async def test_build_case_analysis_wraps_answer_context(monkeypatch) -> None:
    ...
    async def fake_build_answer_context_payload(_deps, answer_params):
