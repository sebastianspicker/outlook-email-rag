...
class _FakeDB:
    ...
    def top_contacts(self, email: str, limit: int = 20) -> list[dict]:
        return [
            {"partner": "bob@example.com", "total_count": 42},
            {"partner": "carol@example.com", "total_count": 10},
        ]
...
def test_run_volume_prints_bars():
    db = _FakeDB()
    with patch("src.cli.TemporalAnalyzer", _FakeTemporalAnalyzer, create=True):
...
def test_run_suggest_prints_suggestions():
    ...
    with patch("src.cli._get_email_db", return_value=fake_db):
