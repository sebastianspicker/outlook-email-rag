...
def db_with_email(db):
    ...
    db.conn.execute(
        """INSERT INTO emails (uid, message_id, subject, sender_name, sender_email,
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            "test-uid-1", "<msg1@test.com>", "Test Subject", "Alice",
            "alice@test.com", "2024-01-15", "Inbox",
...
def test_schema_v4_creates_custody_chain_table(db):
    ...
    row = db.conn.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='custody_chain'"
    ).fetchone()
...
def test_add_evidence_creates_custody_event(db_with_email):
    ...
    result = db_with_email.add_evidence(
        "test-uid-1", "harassment", "important evidence", "test summary", 4,
    )
    events = db_with_email.get_custody_chain(
        target_type="evidence", target_id=str(result["id"])
    )
    assert len(events) >= 1
    event = events[0]
    assert event["action"] == "evidence_add"
...
def test_add_evidence_computes_content_hash(db_with_email):
    ...
    result = db_with_email.add_evidence(
        "test-uid-1", "discrimination", "evidence text", "summary", 3,
    )
    assert "content_hash" in result
    assert len(result["content_hash"]) == 64

    # Verify stored in DB
    row = db_with_email.conn.execute(
        "SELECT content_hash FROM evidence_items WHERE id=?", (result["id"],)
    ).fetchone()
...
def test_add_evidence_content_hash_is_deterministic(db_with_email):
    ...
    result = db_with_email.add_evidence(
        "test-uid-1", "harassment", "evidence text", "summary", 3,
    )
...
def test_update_evidence_logs_custody_event(db_with_email):
    ...
    result = db_with_email.add_evidence(
        "test-uid-1", "harassment", "important evidence", "old summary", 3,
    )
    eid = result["id"]
    ...
    events = db_with_email.get_custody_chain(
        target_type="evidence", target_id=str(eid), action="evidence_update"
    )
    assert len(events) >= 1
    event = events[0]
...
def test_update_evidence_recomputes_content_hash(db_with_email):
    ...
    result = db_with_email.add_evidence(
        "test-uid-1", "harassment", "old quote", "summary", 3,
    )
    ...
    assert row["content_hash"] == EmailDatabase.compute_content_hash(
        "test-uid-1|harassment|new quote"
    )
...
def test_remove_evidence_logs_custody_event(db_with_email):
    ...
    result = db_with_email.add_evidence(
        "test-uid-1", "discrimination", "the evidence quote", "summary", 4,
    )
    eid = result["id"]
    ...
    events = db_with_email.get_custody_chain(
        target_type="evidence", target_id=str(eid), action="evidence_remove"
    )
    assert len(events) >= 1
    event = events[0]
...
def test_email_provenance_includes_ingestion_run(db_with_email):
    ...
    run_id = db_with_email.record_ingestion_start(
        "test.olm", olm_sha256="abcdef", file_size_bytes=1234
    )
...
def test_evidence_provenance_returns_full_chain(db_with_email):
    ...
    result = db_with_email.add_evidence(
        "test-uid-1", "harassment", "important evidence", "summary", 5,
    )
