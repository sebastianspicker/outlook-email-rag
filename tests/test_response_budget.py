...
class TestEvidenceCompactMode:
    ...
    def _make_evidence_item(
        self,
        quote=(
            "A long key quote that exceeds eighty characters for testing"
            " the preview truncation behavior in compact mode."
        ),
    ...
    def test_compact_strips_quote_adds_preview(self):
        ...
        assert "key_quote" not in items[0]
        assert "quote_preview" in items[0]
        assert items[0]["quote_preview"].endswith("...")
        assert len(items[0]["quote_preview"]) == 83  # 80 + "..."
    ...
    def test_compact_strips_notes_and_content_hash(self):
        ...
        assert "content_hash" not in items[0]
    ...
    def test_compact_preserves_short_quote(self):
        ...
        assert items[0]["quote_preview"] == "Short quote"
        assert not items[0]["quote_preview"].endswith("...")
        ...
        for i in range(10):
            conn.execute(
                """INSERT INTO evidence_items
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (f"uid{i}", "harassment", f"quote {i}", f"summary {i}", 3,
                 "Alice", "alice@example.com", f"2024-01-{i+1:02d}",
                 "bob@example.com", f"Subject {i}", "", 1),
            )
        conn.commit()
        return conn
...
class TestCustodyCompactMode:
    def test_compact_strips_details(self):
        events = [
            {
                "id": 1, "action": "evidence_add",
                "details": {"email_uid": "uid1", "category": "bossing"},
                "content_hash": "abc123", "timestamp": "2024-01-01",
            },
            {
                "id": 2, "action": "evidence_update",
                "details": {"old": "val"},
                "content_hash": "def456", "timestamp": "2024-01-02",
            },
        ]
...
class TestJsonResponseSizeGuard:
    ...
    def test_non_dict_response_hard_truncated(self):
        from src.tools.utils import json_response
