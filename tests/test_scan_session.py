...
class TestCandidates:
    ...
    def test_flag_overwrites_existing(self):
        from src.scan_session import flag_candidates, get_session

        flag_candidates("test", ["uid1"], label="maybe", phase=1)
        added = flag_candidates("test", ["uid1"], label="bossing", phase=2)
        assert added == 0  # not newly added
        s = get_session("test")
        assert s.candidates["uid1"].label == "bossing"
        assert s.candidates["uid1"].phase == 2
...
class TestFilterSeenThreadSafety:
    ...
    def test_concurrent_filter_seen_no_duplicates(self):
        ...
        def run_a():
            new, meta = filter_seen(scan_id, batch_a)
            results_a.extend(r["uid"] for r in new)

        def run_b():
            new, meta = filter_seen(scan_id, batch_b)
